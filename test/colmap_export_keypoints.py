...
def main():
    ...
    for image_id, image_name in images:
        ...
        cursor.execute("SELECT data, cols FROM keypoints WHERE image_id=?;",
                       (image_id,))
        row = next(cursor)
        if row[0] is None:
            keypoints = np.zeros((0, row[1]), dtype=np.float32)
        else:
            keypoints = np.fromstring(row[0], dtype=np.float32).reshape(-1, row[1])
            keypoints[:, :2] -= 0.5
