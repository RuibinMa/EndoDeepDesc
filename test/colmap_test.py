...
if __name__ == "__main__":
    ...
    fname = '/home/ruibinma/Desktop/frame012408.jpg---frame012448.jpg.bin'
    matches = read_matrix(fname, np.uint32)
    print(matches)
    print(len(matches))
    
    database_name = '/home/ruibinma/local-feature-evaluation/data/database.db'

    connection = sqlite3.connect(os.path.join(database_name))
    cursor = connection.cursor()    
    images = {}
    cursor.execute("SELECT name, image_id FROM images;")
    for row in cursor:
    ...
    id1 = images['frame012408.jpg']
    id2 = images['frame012448.jpg']
    image_pair_id = image_ids_to_pair_id(id1, id2)
    print(id1,id2)
    
    cursor.execute("SELECT data, rows, cols FROM matches WHERE pair_id = %d;"%image_pair_id)
    tuple = next(cursor)
    row = tuple[1]
    col = tuple[2]
    #matches2 = np.array(matches2)
    matches2 = np.frombuffer(tuple[0], dtype = np.uint32).reshape(-1, col)
    print(matches2-matches)
    
    print(len(np.unique(matches[:, 1])))
    print(len(matches))
    
    print(np.min(matches, axis=0))
    print(np.max(matches, axis=0))
    
    
    keypoints1 = read_matrix('/home/ruibinma/local-feature-evaluation/data/keypoints/frame012408.jpg.bin', np.float32)
    keypoints2 = read_matrix('/home/ruibinma/local-feature-evaluation/data/keypoints/frame012448.jpg.bin', np.float32)
    ...
    for i in range(len(matches)):
        n1 = matches[i, 0]
        n2 = matches[i, 1]
        print(keypoints1[n1, 0:2], keypoints2[n2, 0:2])
...
#     matches2 = dict(
#         (pair_id, blob_to_array(data, np.float32, (-1, 2)))
#         for pair_id, data in db.execute(
#             'SELECT pair_id, data FROM matches WHERE pair_id = %d'%image_pair_id))
